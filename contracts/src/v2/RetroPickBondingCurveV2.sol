...
import {IRetroPickLaunchFactoryGraduationV2} from "./interfaces/IRetroPickGraduationV2.sol";

/**
 * @title RetroPickBondingCurveV2
 ...
 */
contract RetroPickBondingCurveV2 is ReentrancyGuard {
    ...
    IRetroPickFeePolicyV2 public immutable feePolicy;
     ...
     * @param factory_ RetroPickLaunchFactoryV2 address, the only caller allowed through `onlyFactory`.
    ...
    constructor(
        ...
        address deployer_,
        address factory_,
        IRetroPickFeePolicyV2 feePolicy_,
        ...
        deployer = deployer_;
        // Passed explicitly rather than read from msg.sender: RetroPickLaunchFactoryV2
        // deploys this curve indirectly through RetroPickLaunchDeployerV2 to keep its
    ...
    /**
     * @notice Tradeable quote reserve only, matching IRetroPickBondingCurveV2.
     */
    ...
    /**
     * @notice Tradeable token reserve only, matching IRetroPickBondingCurveV2.
     */
        ...
        returns (uint256 tokensOut)
    {
        ...
        if (tokensOut > sellable) {
            ...
            uint256 net = RetroPickBondingCurveMathV2.getAmountIn(sellable, quoteReserveBefore, tokenReserveBefore, 0);
            spent = Math.min(
    ...
    function _tryAutoGraduate() private {
        if (readyToGraduate()) {
            try IRetroPickLaunchFactoryGraduationV2(factory).graduate(token) {}
            catch {
    ...
    function _sweepFees(uint256 minBuybackTokensOut, bool executeBuyback) private {
        ...
        if (buybackAmount != 0) {
            ...
            if (reserveMovementBps <= maxInternalPriceImpactBps) {
                ...
                uint256 tokensOut =
                    RetroPickBondingCurveMathV2.quoteAmountOut(buybackAmount, quoteReserve_, tokenReserve_, 0);
     ...
     * RetroPickFeeEscrowV2, and a permissioned quote asset can stop delivering to
     ...
     *
     * Mirrors RetroPickMemeHookV2.rescuePoolFees for the post-graduation pool and
