 ...
 * "single deployment for every token" pattern RetroPickFeeEscrowV2 already uses.
 *
 ...
 */
contract RetroPickBuybackVaultV2 is Ownable2Step, ReentrancyGuard {
    ...
    function _isAuthorizedLocker(address token, address caller) private view returns (bool) {
        ...
        return caller == IRetroPickLaunchFactoryV2(factory).getLaunchedToken(token).curve;
    }
}
