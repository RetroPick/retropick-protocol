...
import {
    IRetroPickDoorwayV2,
    IRetroPickDoorwayRegistryV2,
    IRetroPickDoorwayVaultV2,
    MigrationDirection,
    MigrationStatus,
    Migration,
    Attestation
} from "./interfaces/IRetroPickDoorwayV2.sol";

/**
 * @title RetroPickDoorwayV2
 ...
 * throughout the RetroPick V2 architecture:
 *
 ...
 * baseline; see docs/launchpad/SECURITY_MODEL.md):
 ...
 */
contract RetroPickDoorwayV2 is IRetroPickDoorwayV2, IRetroPickDoorwayRegistryV2, IRetroPickDoorwayVaultV2 {
    // =============================================================
    //                         CONFIGURATION
    // =============================================================
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "RetroPickDoorwayV2: not owner");
        _;
    }

    modifier onlyRelayer() {
        require(msg.sender == relayer, "RetroPickDoorwayV2: not relayer");
        _;
    }

    modifier onlyGuardian() {
        require(msg.sender == guardian, "RetroPickDoorwayV2: not guardian");
        _;
    }

    modifier doorwayOpen() {
        require(doorwayActive, "RetroPickDoorwayV2: paused");
        _;
    }
    ...
    constructor(address _treasury, address _relayer, address _guardian) {
        require(_treasury != address(0), "RetroPickDoorwayV2: zero treasury");
        require(_relayer != address(0), "RetroPickDoorwayV2: zero relayer");
        require(_guardian != address(0), "RetroPickDoorwayV2: zero guardian");
    ...
    function migrateToSolana(
        ...
        require(supportedMonadTokens[token], "RetroPickDoorwayV2: token unsupported");

        require(supportedSolanaMints[solanaMint], "RetroPickDoorwayV2: mint unsupported");

        require(amount > 0, "RetroPickDoorwayV2: zero amount");
    ...
    function requestFromSolana(
        ...
        require(supportedMonadTokens[monadToken], "RetroPickDoorwayV2: token unsupported");

        require(!usedSourceTransactions[sourceTxHash], "RetroPickDoorwayV2: source tx used");
    ...
    function attestMigration(Attestation calldata attestation) external onlyGuardian {
        ...
        require(migration.status == MigrationStatus.REQUESTED, "RetroPickDoorwayV2: invalid status");

        require(attestation.amount == migration.amount, "RetroPickDoorwayV2: amount mismatch");

        require(
            attestation.sourceTxHash == migration.sourceTxHash || migration.sourceTxHash == bytes32(0),
            "RetroPickDoorwayV2: source mismatch"
        );

        require(!processedAttestations[attestation.sourceTxHash], "RetroPickDoorwayV2: already attested");
    ...
    function executeMigration(bytes32 migrationId, bytes32 destinationTxHash) external onlyRelayer doorwayOpen {
        ...
        require(migration.status == MigrationStatus.ATTESTED, "RetroPickDoorwayV2: not attested");
    ...
    function cancelMigration(bytes32 migrationId) external {
        ...
        require(migration.initiator == msg.sender || msg.sender == guardian, "RetroPickDoorwayV2: not authorized");

        require(
            migration.status == MigrationStatus.REQUESTED || migration.status == MigrationStatus.ATTESTED,
            "RetroPickDoorwayV2: cannot cancel"
        );

        // block-timestamp: CANCELLATION_DELAY is coarse (minutes+); miner timestamp drift of seconds cannot bypass the lock.
        // forge-lint: disable-next-line(block-timestamp)
        require(block.timestamp >= migration.createdAt + CANCELLATION_DELAY, "RetroPickDoorwayV2: cancellation locked");
    ...
    function setRelayer(address newRelayer) external onlyOwner {
        require(newRelayer != address(0), "RetroPickDoorwayV2: zero relayer");
    ...
    function setGuardian(address newGuardian) external onlyOwner {
        require(newGuardian != address(0), "RetroPickDoorwayV2: zero guardian");
    ...
    function setFee(uint256 newFeeBps) external onlyOwner {
        require(newFeeBps <= MAX_FEE_BPS, "RetroPickDoorwayV2: fee too high");
    ...
    function transferOwnership(address newOwner) external onlyOwner {
        require(newOwner != address(0), "RetroPickDoorwayV2: zero owner");
