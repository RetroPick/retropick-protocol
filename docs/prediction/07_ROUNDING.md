...
`PredictionMarket._redeem` with INVALID numerators `(1, 1)` and denominator 2 is that cumulative floor. The Foundry replay of the same compositions matched it: 131087 states, 2097154 transitions, 0 mismatches, 79.76s, gas 370972939410. `block_gas_limit` 600000000000 is Foundry profile `invalid_floor_compositions` only. The default profile does not set it. A re-run on that profile passed 2 tests and 0 failed with the same counts in 79.63s. The default suite skips the walk and still runs the supply-2 test. The named profile still executes the walk: 131087 states, 2097154 transitions, 0 mismatches. Supply 2 pays 0 then 1 on each side. The per-call sequence 0 then 0 is the Python counterexample. PRED-MATH-1 stays partial. The kernel stays a research candidate. Log: `evidence/research/prediction/invalid-floor-compositions-2026-09-26.json`.
