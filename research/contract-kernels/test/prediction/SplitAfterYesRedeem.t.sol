...
/// @notice Split 4, resolve YES_WIN, redeemYes(1), then attempt split(1).
/// @dev split, redeemYes, liability, and the payout formula are not edited.
contract SplitAfterYesRedeemTest is Test {
    ...
    function setUp() public {
        ...
        market = new PredictionMarket(
            address(collateral), resolver, dustSink, bytes32("split-after-yes"), "Yes", "YES", "No", "NO"
        );
        market.activate();
    ...
    function test_split_after_partial_yes_redeem_reverts() public {
        vm.prank(alice);
        market.split(4);
