...
contract DeepRebaseSettlementToken {
...
contract DeepRebasingSettlementFundingTest is Test {
    function test_rebase_below_floor_reverts_redeem() public {
        DeepRebaseSettlementToken token = new DeepRebaseSettlementToken();
        ...
        token.rebaseDown(address(book), 2);
        assertEq(token.balanceOf(address(book)), 0);
        assertTrue(book.redeemable());
        assertEq(book.paidRaw(), 0);

        vm.prank(holder);
        vm.expectRevert(abi.encodeWithSelector(CandidateCumulativeSettlement.PayoutExceedsBalance.selector, 1, 0));
        book.redeem(2);
