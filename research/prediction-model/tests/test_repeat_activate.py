"""A second activate after the market is already OPEN."""
...
def python_witness() -> dict[str, object]:
    ...
    return {
        ...
        "before": before,
        "error": error,
        "function": "PredictionMarket.activate",
        "module": "research/prediction-model/market.py",
    }


class RepeatActivateTests(unittest.TestCase):
    def test_second_activate_leaves_open_books_at_zero(self):
        ...
        rejected = (
            observed["error"] == "activate only from DRAFT"
            ...
            and observed["before"]["state"] == "OPEN"
        )
        ...
        solidity_held = (
            ...
            and solidity.get("before", {}).get("state") == "OPEN"
        )
        ...
        report: dict[str, object] = {
            ...
            "rule": "a second activate while OPEN reverts and collateral, YES supply, and NO supply stay 0",
