...
T-LC-002 says canonical final PRISM resolution cannot be committed twice. The oracle is `PrismSeries.resolve`. Exact enumeration tries every series state and both copies of the four terminal indices. Only `RESOLUTION_PENDING` accepts a commit, and that call accepts one. The series moves to `RESOLVED` and later calls leave the payout in place. An API series commits terminal 2 at payout 1. After component 1 resolves to 1, inconsistent terminals do not commit, terminal 0 commits payout 2/5, and a second call does not change it. A zero-supply series then moves through `make_redeemable` to `REDEEMABLE` and `archive` to `ARCHIVED` without changing payout 1, and resolve is rejected in both states. `activate`, `start_resolution`, and `resolve_component` after commitment are rejected. RESOLVED and REDEEMABLE stay distinct. Classification: **PROVEN_UNDER_ASSUMPTIONS**. No counterexample. Runtime 0.000605 seconds. Canonical MATH-1 stays FAIL because the per-call settlement floor still fails. Log: `evidence/research/prism/t-lc-002-2026-09-26.json`.

T-PARTIAL-001 says partial-resolution NAV decomposes linearly by resolved and unresolved sets. The written identity is the sum of weight times resolved payout over the resolved set plus weight times supplied mark over the unresolved set. SymPy 1.14.0 shows that gap is 0, and that the same value is linear in the weights, for every subset of 1..4 components. Z3 5.1.0 reports unsat for a nonnegative counterexample to the split and unsat for a nonnegative counterexample to weight linearity. `partial_resolution_nav` matches the formula on all 30 of those partitions. The basket 3/5 and 2/5 with resolved payout 1 and mark 3/10 returns 18/25. A missing component or an overlap raises. A negative mark raises and stays outside the nonnegative domain. Classification: **PROVEN_UNDER_ASSUMPTIONS**. No counterexample. Runtime 0.057204 seconds. This is a valuation identity for supplied marks, not an exchange price. Canonical MATH-1 stays FAIL because the per-call settlement floor still fails. Log: `evidence/research/prism/t-partial-001-2026-09-26.json`.

The 6/8/18 precision matrix checks `FixedPointSettlement` and `CumulativeFloorSettlement` without changing either payout formula. The domain is decimals 6, 8, and 18; supplies 0, 1, 2, 2^8, 2^16, 2^32, and 2^64; payouts 0, 1, D/2, D-1, D, D+1, 2^128, and 2^256-1. D = 10^18 * 10^(18-decimals). That is 168 cells and not a uint256 enumeration. Zero supply rejects a one-unit redemption and, at exact ceil funding, leaves balance 0. No sweep runs. Supply 1 pays the one-shot floor and leaves dust 0 or 1. `CX-FP-SETTLEMENT-001` reproduces at decimals 6, 8, and 18: supply 2, payout D-1, receipts 0 and 0, one-shot floor 1, required funding 2, dust 2. The same per-call underpayment appears on 48 cells. The smallest cell in the domain is supply 2, decimals 18, payout 5*10^17, which pays 0 against one-shot 1. That is the same defect, not a new economic rule. The cumulative floor matches the one-shot floor on every cell and its ceil residual stays in {0, 1}. Classification of the matrix: **EXHAUSTIVELY_VERIFIED_WITHIN_DOMAIN**. No new counterexample. Runtime 0.001788 seconds. Canonical MATH-1 stays FAIL because the per-call settlement floor still fails. Log: `evidence/research/prism/precision-boundary-6-8-18-2026-09-26.json`.

The same 168 cells were compared with `CandidateCumulativeSettlement`. Cells where `supply * payout_wad` fits in uint256 are fixtures. Forge 1.8.3, solc 0.8.26, optimizer enabled, 200 runs, via IR off, ran `forge test --match-contract CandidatePrecisionBoundaryDifferentialTest -vv` and passed 1 test with 0 failures. No mismatch. 129 cumulative cells matched. 15 cells are outside the Solidity domain because the product does not fit in uint256. 24 zero-supply cells are outside it because the constructor reverts `ZeroSupply`. `CX-FP-SETTLEMENT-001` at decimals 6, 8, and 18 remains a Python regression of `FixedPointSettlement.redeem`. No Solidity function implements that per-call rule, and it was not added. The kernel stays `differential_research_kernel`. Canonical MATH-1 stays FAIL because the per-call settlement floor still fails. Log: `evidence/research/prism/precision-boundary-solidity-2026-09-26.json`.

The zero-supply settlement residual was classified from the existing functions. Z3 5.1.0 reports unsat for a nonnegative numerator and a positive denominator whose ceil-floor gap is outside {0, 1}. Under exact ceil funding that gap is what remains after the cumulative floor is paid. A book constructed at supply 0 is accepted in Python. All 24 precision-boundary zero-supply cells fund at 0, reject a one-unit redemption, and leave balance 0. `CandidateCumulativeSettlement` still reverts `ZeroSupply` on an empty holder list. After supply 2, payout `10^18-1`, decimals 18, and funding 2 are fully redeemed, the candidate pays 0 then 1 and leaves token balance 1. A further `redeem(1)` reverts and the balance stays 1. Payout 0 leaves 0. Funding 5 leaves 4, which is surplus above the ceil and also stays. `FixedPointSettlement.sweepable_dust` reads the per-call leftover 2 and does not transfer it. That leftover remains `CX-FP-SETTLEMENT-001`. `PrismSeries.archive` leaves `settlement_balance` unchanged: redeeming supply 2 at payout 1 from balance 5 pays 2, and archive leaves 3. `FixedPointSeries.sweep_dust` returns component backing and zeros it; that is not the settlement residual. No settlement function withdraws more than the 0-or-1 gap. Sweep policy: **NOT_YET_VALIDATED**. Residual bound: **PROVEN_UNDER_ASSUMPTIONS**. No extraction witness. Runtime 0.018452 seconds. Forge 1.8.3, solc 0.8.26, optimizer enabled, 200 runs, via IR off, ran `forge test --match-contract CandidateZeroSupplyDustTest -vv` and passed 4 tests with 0 failures. The kernel stays `differential_research_kernel`. Canonical MATH-1 stays FAIL. Log: `evidence/research/prism/zero-supply-dust-2026-09-26.json`.

`CandidateCumulativeSettlementInvariantTest` is a stateful Foundry suite for that same kernel. The handlers are `fund`, `makeRedeemable`, and `redeem`. `fund` mints the settlement token into the book, which is the existing funding path. Redeem quantities are bounded by the holder balance. Known revert paths return before the call, and `fail_on_revert` is false. The fixture is supply 8, amounts 3 and 5, payout `10^18-1`, decimals 18. The five invariants are: redeemed units never exceed the constructed supply; `paidRaw` equals the cumulative floor of the redeemed cursor; the token balance equals funding minus `paidRaw`; after full redemption the leftover equals that same difference; when funding equals the exact ceil, that leftover is 0 or 1. A warmup of 32 runs and depth 16 passed on seeds 20260926 and 20260927 with 512 calls and 0 reverts. The recorded campaign raised that to 256 runs and depth 128. Both seeds passed: 32768 calls, 0 reverts, 0 discards, about 2.06s and 2.09s. Forge 1.8.3, solc 0.8.26, optimizer enabled, 200 runs, via IR off. No counterexample. No regression was added. Sweep policy stays **NOT_YET_VALIDATED**. The kernel stays `differential_research_kernel`. Canonical MATH-1 stays FAIL. Log: `evidence/research/prism/candidate-settlement-invariant-2026-09-26.json`.

`CandidateComponentBackingInvariantTest` targets `deposit`, `mint`, and `redeem` on the existing backing kernel. Weights stay `WAD/2` and `WAD/2`. The post-mint requirement is the contract's `requiredRaw`. Handlers return before a known revert, and a rejected mint or redeem is caught and checked so supply and backing stay put. `fail_on_revert` is false. Supply increases only through a successful mint. After every successful call, each `backingRaw` entry is at least `requiredRaw(supply)`. A probe token records supply while backing tokens leave the kernel; that supply is already the post-redeem supply. Warmup 32 runs and depth 16 passed on seeds 20260926 and 20260927 with 512 calls and 0 reverts. The recorded campaign is 256 runs and depth 128: 32768 calls, 0 reverts, 0 discards, about 4.02s and 3.95s. Forge 1.8.3, solc 0.8.26, optimizer enabled, 200 runs, via IR off. No counterexample. The kernel stays `differential_research_kernel`. Canonical MATH-1 stays FAIL. Log: `evidence/research/prism/candidate-backing-invariant-2026-09-26.json`.

`CandidateReservationLedgerInvariantTest` targets `deposit` and `reserve`. The contract has no `release` and no `withdraw`, and this suite does not add them. `fail_on_revert` is false. For each of two assets, the sum of reservations across the handler's series ids equals `totalReserved`, that sum is at most the physical balance, and `available` equals balance minus total reserved. An empty series id and a reserve above available are caught, and the ledger matches the pre-call balance, total reserved, and series bucket. The same warmup passed. The recorded campaign is 256 runs and depth 128 on seeds 20260926 and 20260927: 32768 calls, 0 reverts, 0 discards, about 2.35s and 2.41s. No counterexample. The kernel stays `differential_research_kernel`. Canonical MATH-1 stays FAIL. Log: `evidence/research/prism/candidate-reservation-invariant-2026-09-26.json`.
